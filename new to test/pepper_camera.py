...
class PepperCamera:
    ...
    def get_depth_image(self, x, y):
        ...
        try:
            ...
            self.video_proxy.setActiveCamera(0) # Check if necessary?
    ...
    def get_depth_at_pixel(self, depth_x, depth_y):
        try:
            # Get depth iamge
            depth_image, width, height = self.get_depth_image()

            # Get depth from small region
            region_size = 3  # 3x3 pixel region
    ...
    def get_3d_position(self, x_cen, y_cen):
        ...
        angular_position = self.video_proxy.getAngularPositionFromImagePosition("CameraTop", [x_cen, y_cen])
        ...
        camera_transform = self.motion_proxy.getTransform("CameraTop", 0, 2)
    ...
    # def pixel_to_normalised_coords(self, x, y, camera_matrix):
    #     # Convert pixel coordinates to normalised coordinates
    #     try: 
    #         inv_K = np.linalg(camera_matrix)
    #         pixel_coords = np.array([x, y, 1]).reshape(3, 1) # Homogenous coordinates
    #         normalised_coords = np.dot(inv_K, pixel_coords)
    #         return normalised_coords
    #     except Exception as e:
    #         logger.error("Error converting pixel to normalised coords: {}".format(e))
    #         return None
        
    # def pixel_to_3d(self, x, y, depth):
    #     # Convert pixel coordinates and depth to 3D coordinates
    #     try:
    #         X = (x - self.c_x) * depth / self.f_x
    #         Y = (y - self.c_y) * depth / self.f_y
    #         Z = depth # Depth from depth map

    #         return X, Y, Z
        
    #     except Exception as e:
    #         logger.error("Error converting pixel to 3D: {}".format(e))
    #         return None, None, None
        
    # def get_object_position(self, top_x, top_y):
    #     # Get 3D position of object
    #     try:
    #         # Map co-ordinates from top to depth camera
    #         depth_x, depth_y, = self.map_to_depth_cam(top_x, top_y)
    ...
    #         width = depth_frame[0]
    #         height = depth_frame[1]
    #         depth_data = np.frombuffer(depth_frame[6], dtype=np.uint16).reshape((height, width))

    #         if depth_x < 0 or depth_x >= width or depth_y < 0 or depth_y >= height:
    #             logger.warning("Coordinates (", depth_x, ", ", depth_y,") out of bounds. Image size: ", width, "x", height)
    #             return None, None, None
    ...
    #         depth_region = depth_data[y_start:y_end, x_start:x_end]
    ...
    #         X, Y, Z = self.pixel_to_3d(top_x, top_y, depth)
    ...
    #         return depth, (X, Y, Z)
        ...
        # except Exception as e:
        #     print("Error getting depth and position: {}".format(e))
        #     return None, None, None
